function ret = plotVxRange(fName, midpoint, width, sliceHot, sliceCold)
   ...
   for i=1:numColdCulled
       ...
       if tmpX >= midpoint - width && tmpX <= midpoint + width
           ...
           coldP(1,nextSpace) = tmpVx;
           nextSpace = nextSpace + 1;
       end
       % Skip the  remaining velocity plus whatever else I need to reach
       % the next particle I care about
       skipBytes = sizeOfFloat * 2 + sizeOfFloat * 5 * (sliceCold-1);
       fseek(f, skipBytes, 'cof');
   end
   ...
   title(strcat([fields{1} ' cold ' fields{2} ...
       ' Vx (x=' windowBegin ' to x=' windowEnd ')']));
   xlabel('vx');
   ylabel('y');
   %axis([-2.5 2.2 0 yMax]);
   ...
   fields = strsplit(fName, '_');
   outFile = strcat(fields{1}, '_cold_vx_vs_y_x', ...
   ...
   fields = strsplit(fName, '_');
   outFile = strcat(fields{1}, '_hot_vx_vs_y_x', ...
       windowBegin, '-', windowEnd, '_', fields{2});
   print('-dpng', outFile);
